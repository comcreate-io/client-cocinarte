...
import {
  ChefHat, Calendar, Clock, User, CheckCircle, AlertCircle,
  Loader2, Baby, Shield, FileCheck, ArrowRight, RotateCcw
} from 'lucide-react'
...
export default function GuestFormPage() {
  ...
  const fetchFormData = async () => {
    try {
      setLoading(true)
      const res = await fetch(`/api/guest-booking/form-data?token=${token}`)
      ...
      if (data.already_completed) {
        ...
        setGuestBooking(data.guest_booking)
        return
      }
      ...
      // Pre-fill guest parent name and child name from booking
      setGuestParentName(data.guest_booking.guest_parent_name || '')
      setChildFullName(data.guest_booking.guest_child_name || '')
  ...
  const handleReuseChild = (child: GuestChild) => {
    // Pre-fill all fields except consent/signature
      ...
      <div className="min-h-screen bg-gradient-to-b from-orange-50 to-white">
        ...
        <div className="flex items-center justify-center min-h-[60vh] pt-32">
          <div className="text-center">
            ...
            <p className="text-slate-600 text-lg">Loading enrollment form...</p>
          </div>
        </div>
        <CocinarteFooter />
      </div>
          ...
          <Card className="max-w-md w-full mx-4">
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-slate-600 mb-2">
                This enrollment form for <strong>{guestBooking?.guest_child_name}</strong> has already been submitted.
              </p>
              <p className="text-sm text-slate-500">
                Completed on {guestBooking?.form_completed_at
                  ? new Date(guestBooking.form_completed_at).toLocaleDateString()
                  : ''}
              </p>
            </CardContent>
          </Card>
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-orange-50 to-white">
        <CocinarteHeader />
        <div className="flex items-center justify-center min-h-[60vh] pt-32 pb-12">
            ...
            <CardContent className="pt-8 pb-8 text-center">
              ...
              {classDetails && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-left mt-4">
                  ...
                  <p className="text-sm text-blue-700"><strong>Class:</strong> {classDetails.title}</p>
                  <p className="text-sm text-blue-700"><strong>Date:</strong> {formatDate(classDetails.date)}</p>
                  <p className="text-sm text-blue-700"><strong>Time:</strong> {formatTime(classDetails.time)}</p>
                </div>
              )}
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 pt-32">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-slate-800 mb-2">Guest Enrollment Form</h1>
          <p className="text-slate-600">
            Gifted by <strong>{guestBooking?.purchaser_name}</strong>
          </p>
        </div>

        {/* Class Info */}
        {classDetails && (
          <Card className="mb-6 border-[#1E3A8A]/20">
            <CardContent className="pt-6">
              <div className="flex items-start gap-4">
                <div className="bg-[#1E3A8A]/10 p-3 rounded-lg">
                  <ChefHat className="h-6 w-6 text-[#1E3A8A]" />
                </div>
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-slate-800">{classDetails.title}</h3>
                  <div className="flex flex-wrap gap-4 mt-2 text-sm text-slate-600">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {formatTime(classDetails.time)}
                    </span>
                    {classDetails.classDuration && (
                      <span className="flex items-center gap-1">
                        <Clock className="h-4 w-4" />
                        {classDetails.classDuration} min
                      </span>
                    )}
                  </div>
                </div>
          ...
          <Alert className="mb-6 bg-purple-50 border-purple-200">
            ...
            <AlertDescription>
              <p className="font-semibold text-purple-800 mb-2">
                We noticed you've filled out a form for a previous class.
              </p>
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              <Baby className="h-5 w-5 text-[#1E3A8A]" />
              Child Information
            </CardTitle>
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="text-lg">{LIABILITY_CONSENT_TEXT.title} <span className="text-destructive">*</span></CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
        ...
        <div className="flex justify-end pb-12">
          <Button
            ...
            size="lg"
            className="px-8 bg-[#F0614F] hover:bg-[#F0614F]/90 text-white"
          >
            {submitting ? (
              <>
